...
const createRoute =
	...
	({ root, props, actions = [], timestamp, shouldOptimize }) => {
		actions
			...
			.forEach((action) => routes[action.type]({ root, props, action: action.data, timestamp, shouldOptimize }));
		if (fn) {
...
const getDateString = (date = new Date()) =>
	`${date.getFullYear()}-${leftPad(date.getMonth() + 1, '00')}-${leftPad(
		date.getDate(),
		'00',
	)}_${leftPad(date.getHours(), '00')}-${leftPad(date.getMinutes(), '00')}-${leftPad(date.getSeconds(), '00')}`;
...
const processFileChunked = (apiUrl, action, name, file, metadata, load, error, progress, abort, transfer, options) => {
	...
	const onload =
		action.onload || ((xhr, method) => (method === 'HEAD' ? xhr.getResponseHeader('Upload-Offset') : xhr.response));
	const onerror = action.onerror || ((res) => null);
...
const createFileProcessorFunction =
	(apiUrl, action, name, options) => (file, metadata, load, error, progress, abort, transfer) => {
		// no file received
		if (!file) return;
...
const isExternalURL = (url) =>
	(url.indexOf(':') > -1 || url.indexOf('//') > -1) && getDomainFromURL(location.href) !== getDomainFromURL(url);
	...
	PROCESS_ITEM: getItemByQueryFromState(state, (item, success, failure) => {
		...
		item.onOnce('process-complete', () => {
			...
			const allItemsProcessed =
				query('GET_ITEMS_BY_STATUS', ItemStatus.PROCESSING_COMPLETE).length === state.items.length;
...
const getItemHeight = (child) =>
	child.rect.element.height + child.rect.element.marginBottom * 0.5 + child.rect.element.marginTop * 0.5;
const getItemWidth = (child) =>
	child.rect.element.width + child.rect.element.marginLeft * 0.5 + child.rect.element.marginRight * 0.5;
...
const listModified = (root, filename, label) => {
	...
	assist(
		...
		`${label} ${filename}, ${total} ${
			total === 1 ? root.query('GET_LABEL_FILE_COUNT_SINGULAR') : root.query('GET_LABEL_FILE_COUNT_PLURAL')
		}`,
	);
