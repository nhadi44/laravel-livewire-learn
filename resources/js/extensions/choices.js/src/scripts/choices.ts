...
class Choices implements Choices {
	...
	constructor(
		...
		if (this._isTextElement) {
			...
			this.passedElement = new WrappedSelect({
				...
				classNames: this.config.classNames,
				template: (data: Item): HTMLOptionElement => this._templates.option(data),
			});
		}
		...
		if (this.passedElement.value && this._isTextElement) {
			const splitValues: string[] = this.passedElement.value.split(this.config.delimiter);
	...
	getValue(valueOnly = false): string[] | Item[] | Item | string {
		const values = this._store.activeItems.reduce<any[]>((selectedItems, item) => {
	 ...
	 */
	setChoices(
		choicesArrayOrFetcher: Choice[] | Group[] | ((instance: Choices) => Choice[] | Promise<Choice[]>) = [],
		value = 'value',
		label = 'label',
		...
		if (typeof choicesArrayOrFetcher === 'function') {
			...
			if (typeof Promise === 'function' && fetcher instanceof Promise) {
				...
				return new Promise((resolve) => requestAnimationFrame(resolve))
					...
					.then(() => fetcher)
					.then((data: Choice[]) => this.setChoices(data, value, label, replaceChoices))
					.catch((err) => {
						if (!this.config.silent) {
							console.error(err);
						}
	...
	_createGroupsFragment(
		...
		const getGroupChoices = (group): Choice[] =>
			choices.filter((choice) => {
				...
				return (
					choice.groupId === group.id && (this.config.renderSelectedChoices === 'always' || !choice.selected)
				);
			});
	...
	_handleButtonAction(activeItems?: Item[], element?: HTMLElement): void {
		...
		const itemId = element.parentNode && (element.parentNode as HTMLElement).dataset.id;
	...
	_handleItemAction(activeItems?: Item[], element?: HTMLElement, hasShiftKey = false): void {
	...
	_canAddItem(activeItems: Item[], value: string): Notice {
		let canAddItem = true;
		let notice =
			typeof this.config.addItemText === 'function' ? this.config.addItemText(value) : this.config.addItemText;
	...
	_onKeyDown(event: KeyboardEvent): void {
		...
		const { BACK_KEY, DELETE_KEY, ENTER_KEY, A_KEY, ESC_KEY, UP_KEY, DOWN_KEY, PAGE_UP_KEY, PAGE_DOWN_KEY } =
			KEY_CODES;
	...
	_onKeyUp({ target, keyCode }: Pick<KeyboardEvent, 'target' | 'keyCode'>): void {
		...
		if (this._isTextElement) {
			...
			const userHasRemovedValue = wasRemovalKeyCode && target && !(target as HTMLSelectElement).value;
	...
	_onEnterKey(event: KeyboardEvent, activeItems: Item[], hasActiveDropdown: boolean): void {
		...
		const targetWasButton = target && (target as HTMLElement).hasAttribute('data-button');
	...
	_addPredefinedGroups(groups: Group[] | HTMLOptGroupElement[] | Element[]): void {
		...
		const placeholderChoice = (this.passedElement as WrappedSelect).placeholderOption;
			...
			if (this._isSelectElement) {
				...
				if (choice.choices) {
					...
					const shouldPreselect =
						this._isSelectOneElement && !hasSelectedChoice && index === firstEnabledChoiceIndex;
	...
	_generatePlaceholderValue(): string | null {
		if (this._isSelectElement && (this.passedElement as WrappedSelect).placeholderOption) {
