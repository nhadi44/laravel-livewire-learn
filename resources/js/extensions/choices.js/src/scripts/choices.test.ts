	...
	describe('private methods', () => {
		...
		describe('_getTemplate', () => {
			describe('when passing a template key', () => {
				it('returns the generated template for the given template key', () => {
					...
					expect(instance._templates[templateKey]).to.have.been.calledOnceWith(instance.config, customArg);
				});
			});
		});
		...
		describe('_onKeyDown', () => {
			...
			describe('direction key', () => {
				const keyCodes = [KEY_CODES.UP_KEY, KEY_CODES.DOWN_KEY, KEY_CODES.PAGE_UP_KEY, KEY_CODES.PAGE_DOWN_KEY];
			...
			describe('enter key', () => {
				it(`calls _onEnterKey with the expected arguments`, () => {
					...
					expect(instance._onEnterKey).to.have.been.calledWith(event, activeItems, hasActiveDropdown);
				});
			});
			...
			describe('delete key', () => {
				...
				keyCodes.forEach((keyCode) => {
					it(`calls _onDeleteKey with the expected arguments`, () => {
						...
						expect(instance._onDeleteKey).to.have.been.calledWith(event, activeItems, hasFocussedInput);
					});
				});
			});
		});
		...
		describe('_removeItem', () => {
			...
			describe('when given an item to remove', () => {
				...
				it('dispatches a REMOVE_ITEM action to the store', () => {
					...
					expect(instance._store.dispatch).to.have.been.calledWith(removeItem(item.id, item.choiceId));
				});
