...
!(function (e, t) {
			...
			(function (e, t) {
				...
				try {
					for (
						...
						!(r = (o = c.next()).done) && (i.push(o.value), !t || i.length !== t);
						r = !0
					);
				} catch (e) {
					(a = !0), (n = e);
						...
						if (n) {
							...
							if (!y || (y && g)) {
								...
								if ('force' === p) (h = d), (u = f);
									...
									'cover' === p ? (v = Math.max(I, M)) : 'contain' === p && (v = Math.min(I, M)),
										(h *= v),
										(u *= v);
							}
						}
			...
			return e.utils.createView({
				...
				create: function (t) {
					...
					(i.ref.image = i.appendChildView(
						...
						(i.ref.createMarkup = function () {
							i.ref.markup ||
								(i.ref.markup = i.appendChildView(i.createChildView(I(e), Object.assign({}, r))));
						}),
				...
				write: function (e) {
					var t = e.root,
						...
						h = i.height;
					t.ref.image.crop = a;
					var u = { x: 0, y: 0, width: s, height: h, center: { x: 0.5 * s, y: 0.5 * h } },
			...
			self.onmessage = function (e) {
				for (
					...
					(A = r[T] / 255),
						...
						(P = r[T + 2] / 255),
						(C = r[T + 3] / 255),
						(r[T] = Math.max(0, Math.min(255 * (A * n + R * o + P * c + C * s + h), 255))),
						(r[T + 1] = Math.max(0, Math.min(255 * (A * u + R * l + P * d + C * f + p), 255))),
						(r[T + 2] = Math.max(0, Math.min(255 * (A * g + R * m + P * y + C * E + v), 255))),
						(r[T + 3] = Math.max(0, Math.min(255 * (A * w + R * _ + P * I + C * M + x), 255)));
		...
		z = function (e) {
			var t = e.utils.createView({
					...
					create: function (e) {
						...
						G++,
							t.element.classList.add('filepond--image-preview-overlay-'.concat(i.status)),
				...
				n = function (e) {
					...
					if (o) {
						...
						t.query('GET_IMAGE_PREVIEW_MARKUP_SHOW') &&
							((c = o.getMetadata('markup') || []), (s = o.getMetadata('resize')), (l = !0));
			...
			return e.utils.createView({
				...
				create: function (e) {
					...
					(i.ref.images = []),
						...
						(i.ref.imageViewBin = []),
						(i.ref.overlayShadow = i.appendChildView(i.createChildView(t, { opacity: 0, status: 'idle' }))),
								...
								var s,
									...
									d = function (e) {
										...
										if (o && s) {
											...
											var E = S(e, m, y, r),
												v = function () {
													var r = t.query('GET_IMAGE_PREVIEW_CALCULATE_AVERAGE_IMAGE_COLOR')
														? N(data)
														: null;
										...
										if (/crop|markup|resize/.test(r.change.key)) {
											...
											if (
												...
												n({
													...
													image:
														((l = u.image),
														((d = d || document.createElement('canvas')).width = l.width),
														(d.height = l.height),
										...
										if (c && n(c.file) && !c.archived) {
											...
											if (
												...
												if (!(!h && u && s.size > u)) {
													...
													var d =
														!h &&
														s.size > r('GET_IMAGE_PREVIEW_MAX_INSTANT_PREVIEW_FILE_SIZE');
															...
															if (!(a || n || o)) {
																...
																if (s && h) {
																	var u = e.query('GET_IMAGE_PREVIEW_MIN_HEIGHT'),
																		...
																		d =
																			(r.getMetadata('exif') || {}).orientation ||
																			-1;
																	if (d >= 5 && d <= 8) {
																		var f = [h, s];
																		(s = f[0]), (h = f[1]);
																	...
																	var g = h / s,
																		m =
																			(r.getMetadata('crop') || {}).aspectRatio ||
																			g,
	...
	return (
		'undefined' != typeof window &&
			...
			document.dispatchEvent(new CustomEvent('FilePond:pluginloaded', { detail: H })),
		H
	);
});
