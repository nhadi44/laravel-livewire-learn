...
!(function (n) {
	...
	function a(t, e, n) {
		...
		return (
			!!h(t) ||
			((h((a = t)) || 'string' == typeof a) && !!l(t.replace(V, '').replace(/<script/i, ''), e, n)) ||
			null
		);
	}
	function m(t, e, n, a) {
		...
		w = function (t, v) {
			...
			(v = t),
				...
				(this.fnAddData = function (t, e) {
					var n = this.api(!0),
						t = (Array.isArray(t) && (Array.isArray(t[0]) || P.isPlainObject(t[0])) ? n.rows : n.row).add(
							t,
						);
	...
	function i(n) {
		...
		P.each(n, function (t, e) {
			(a = t.match(/^([^A-Z]+?)([A-Z])/)) &&
				...
				((r = t.replace(a[0], a[2].toLowerCase())), (o[r] = t), 'o' === a[1] && i(n[t]));
		}),
			(n._hungarianMap = o);
	}
	function C(n, a, r) {
	...
	function lt(t) {
		for (
			var e, n, a, r, o, i, l, s = t.aoColumns, u = t.aoData, c = w.ext.type.detect, f = 0, d = s.length;
			f < d;
			f++
		)
		...
		if (-1 !== P.inArray(!1, n)) D(t, !1);
			...
			if (0 !== u.length)
				for (var f = s ? t.aoData.length : c, d = s ? 0 : n; d < f; d++) {
					...
					0 !== i &&
						((h = o[r % i]),
						g._sRowStripe != h && (P(b).removeClass(g._sRowStripe).addClass(h), (g._sRowStripe = h))),
	...
	function At(t) {
		for (
			...
			(n = r[g]),
				...
				h('mDataProp_' + g, e),
				i.bFilter &&
					(h('sSearch_' + g, a.sSearch), h('bRegex_' + g, a.bRegex), h('bSearchable_' + g, n.bSearchable)),
	...
	function _e(t, e, n, a, r) {
		for (
			...
			for (
				l = 0, s = (i = e[f] && e[f].split && !e[f].match(/[\[\(:]/) ? e[f].split(',') : [e[f]]).length;
				l < s;
				l++
			)
	...
	function ke(t, e, n, a) {
		...
		return 0 === t || (t && '-' !== t)
			? 'number' == (r = typeof t) || 'bigint' == r
				? t
				: ((t = e ? $(t, e) : t).replace && (n && (t = t.replace(n, '')), a && (t = t.replace(a, ''))), +t)
			: -1 / 0;
	}
	function Me(n) {
		P.each(
	...
	function Ue(t, e, n) {
		...
		if (j.moment) {
			...
			if (
				!(a = e && 'string' == typeof t ? j.luxon.DateTime.fromFormat(t, e) : j.luxon.DateTime.fromISO(t))
					.isValid
			)
				return null;
			a.setLocale(n);
