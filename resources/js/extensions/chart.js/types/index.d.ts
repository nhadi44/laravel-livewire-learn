...
> = DeepPartial<{ [key in ChartType]: ChartTypeRegistry[key]['metaExtensions'] }[TType]> &
...
export declare const Interaction: {
    ...
    /**
     * Helpers function to select candidate elements for interaction
     */
...
export interface Plugin<TType extends ChartType = ChartType, O = AnyObject> extends ExtendedPlugin<TType, O> {
    ...
    beforeEvent?(
        chart: Chart,
        args: { event: ChartEvent; replay: boolean; cancelable: true; inChartArea: boolean },
        options: O
    ): boolean | void;
...
> = DeepPartial<{ [key in ChartType]: { type: key } & ChartTypeRegistry[key]['datasetOptions'] }[TType]> &
