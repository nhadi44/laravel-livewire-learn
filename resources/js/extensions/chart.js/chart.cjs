...
class DatasetController {
	...
	getMinMax(scale, canStack) {
		...
		function _skip() {
			...
			return (
				!helpers_segment.isNumberFinite(parsed[scale.axis]) || otherMin > otherValue || otherMax < otherValue
			);
		}
		for (i = 0; i < ilen; ++i) {
			if (_skip()) {
...
function autoSkip(scale, ticks) {
	...
	if (numMajorIndices > 0) {
		...
		skip(
			...
			newTicks,
			spacing,
			helpers_segment.isNullOrUndef(avgMajorSpacing) ? 0 : first - avgMajorSpacing,
			first,
		);
		...
		skip(
			...
			spacing,
			last,
			helpers_segment.isNullOrUndef(avgMajorSpacing) ? ticks.length : last + avgMajorSpacing,
		);
		return newTicks;
	}
...
class Scale extends Element {
	...
	getPixelForDecimal(decimal) {
		...
		return helpers_segment._int16Range(
			this._alignToPixels ? helpers_segment._alignPixel(this.chart, pixel, 0) : pixel,
		);
	}
...
function needContext(proxy, names) {
	...
	for (const prop of names) {
		...
		if (
			(scriptable && (helpers_segment.isFunction(value) || hasFunction(value))) ||
...
class ArcElement extends Element {
	...
	inRange(chartX, chartY, useFinalPosition) {
		...
		const betweenAngles =
			_circumference >= helpers_segment.TAU || helpers_segment._angleBetween(angle, startAngle, endAngle);
...
class LogarithmicScale extends Scale {
	...
	getLabelForValue(value) {
		return value === undefined
			? '0'
			: helpers_segment.formatNumber(value, this.chart.options.locale, this.options.ticks.format);
	}
	configure() {
	...
	getPixelForValue(value) {
		...
		return this.getPixelForDecimal(
			value === this.min ? 0 : (helpers_segment.log10(value) - this._startValue) / this._valueRange,
		);
	}
	getValueForPixel(pixel) {
...
function getTickBackdropHeight(opts) {
	...
	if (tickOpts.display && opts.display) {
		...
		return (
			helpers_segment.valueOrDefault(tickOpts.font && tickOpts.font.size, helpers_segment.defaults.font.size) +
			padding.height
		);
	}
	return 0;
